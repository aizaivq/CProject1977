...
int main(int a,char**p)
{
	...
	//char yuvbuff[width * height * 3 / 2];
	//unsigned char yuvbuff[width * height];
	FILE * file = fopen("test.yuv","r");
	//fread(yuvbuff,sizeof(unsigned char),width * height *3 / 2,file);
	//
	//
	int x = 0;
	int y = 0;
	...
	Mat yuvMat(h * 3 / 2 ,w,CV_8UC1);
	//Mat yuvMat(height * 3 / 2 ,width,CV_8UC1);
	//yuvMat.data = (unsigned char*)yuvbuff;
	yuvMat.data = clipbuff;
	//        imshow("原图像", yuvMat);




	//Rect rect(128,128,300,300);
	//Mat clipMat = yuvMat(rect);
	Mat rgbMat;  
	...
	//	float qua = atof(meanValueString.c_str());
