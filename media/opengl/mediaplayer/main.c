...
//YUV420P to RGB24  
void CONVERT_YUY2toRGB24(unsigned char* yuv_src,unsigned char* rgb_dst,int nWidth,int nHeight)  
{  
    printf("CONVERT_YUY2toRGB24\n");
    ...
    {  
        ...
        {  
            ...
            int yos = nWidth * i * 2 + j*2;
     //       int uos = nWidth * i * 4 + j*2 + 1;
     //       int vos = nWidth * i * 4 + j*2 + 1;
    ...
    /*        if(os > 100)
       ...
       //     Y = *(y_planar + rgb_width * i *)
          //  offSet = (i>>1) * (u_width) + (j>>1);  
            if(j % 2 != 0)
            {
             int uos = nWidth * i * 2 + j*2 + 1;
             int vos = nWidth * i * 2 + j*2 + 3;
             U = *(yuv_src + uos);
             V = *(yuv_src + vos);
         }
         else
...
void display(void){ 
 ...
 glutSolidSphere(1.0, 20, 16); //
    ...
    CONVERT_YUY2toRGB24(buffer,buffer_convert,pixel_w,pixel_h);  
